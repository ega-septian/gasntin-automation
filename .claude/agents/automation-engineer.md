...
6. Reuse existing helpers before writing new ones (`api/auth.ts`, `api/products.ts`, `api/assets.ts`, `data/users.ts`, `data/products.ts`, `support/jwt.ts`, `support/db.ts`, `support/qase.ts`); a new API domain with no `api/<domain>.ts` file yet gets its own file, and a new reusable payload factory belongs in `data/<domain>.ts` (see `.claude/rules/data-factories-in-data-folder.md`), never inline in the spec.
7. Write/update the test, then run it locally with the Qase reporter disabled (prefix the command with `QASE_API_TOKEN=`) unless the user explicitly asked for a synced run — generating/verifying a test must never silently create a new Run in Qase. Before reporting the work done, also run `npm run format:check` and `npm run typecheck` — these are the same `lint` job checks CI runs on every PR, so a formatting/type issue caught here is one that would otherwise fail CI after the fact. If `format:check` fails, fix it with `npx prettier --write <file>` (never hand-format to match Prettier's style); if `typecheck` fails, fix the actual type error, not by weakening types (`any`, unnecessary `as` casts) to silence it.
